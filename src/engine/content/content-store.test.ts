...
describe('ContentStore (quota-aware)', () => {
  ...
  it('refuses content that would exceed quota — cleanly, without partial writes', async () => {
    const store = new ContentStore(2000, 1024);
    const res = await store.storeContent(new Uint8Array(5000));
    expect(res.ok).toBe(false);
    expect(res.reason).toMatch(/quota/);
    expect(store.used()).toBe(0); // nothing written
  });

  it('never writes a blob larger than the chunk size', async () => {
    const store = new ContentStore(1_000_000, 1024);
    const res = await store.putBlock(cidOf(new Uint8Array(2048)), new Uint8Array(2048));
    expect(res.ok).toBe(false);
    expect(res.reason).toMatch(/chunk size/);
  });

  it('dedups identical chunks (content-addressed)', async () => {
    const store = new ContentStore(1_000_000, 1024);
    const data = new TextEncoder().encode('repeatme'.repeat(300));
    await store.storeContent(data);
    const usedAfterFirst = store.used();
    await store.storeContent(data); // same content again
    expect(store.used()).toBe(usedAfterFirst); // no extra space
  });

  it('handles a 100 MB file via chunking, and rejects it cleanly when quota is too small', async () => {
    ...
    const ok = await big.storeContent(buf);
    expect(ok.ok).toBe(true);
    expect((await big.getContent(ok.manifest!))!.length).toBe(SIZE);

    // too little quota → clean failure, no crash, nothing written
    const small = new ContentStore(50 * 1024 * 1024);
    const fail = await small.storeContent(buf);
