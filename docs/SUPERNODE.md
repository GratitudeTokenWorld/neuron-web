...
| `GET /providers?network=&limit=` | storage providers this archive knows: each one's latest `storage-register` + `storage-heartbeat`, hex-encoded, plus recent `storage-deregister` TOMBSTONES so a departure actually reaches every node (omitting the provider is not enough — clients take the union across relays, so one relay still serving the stale register would resurrect it). Bounded (default 20, cap 50); departures do not consume a slot and stop being served after 2 x MAX_OFFLINE | each block verified client-side (`foldProviderBlocks`) — the relay serves the PROVIDER's own signatures, so it can choose what to show but cannot invent, inflate or forge. Ask several relays for the union |
| `GET /files?network=[&cid=][&owner=][&withdrawn=1][&limit=]` | file records this archive holds, plus `total` (**this archive's** count, never a network total). Bounded (default 50, cap 200). Closed the last `O(N)` in storage: clients used to ingest a global `files` topic and persist a record for every file on the network | each record verified client-side (`foldFileRecords`), which REBUILDS the signed payload from the record's own fields — so a relay cannot invent a file, inflate a size, or attribute one to the wrong account. Withdrawals are served as tombstones (`removed: true`), because absence cannot tell a holder a file was withdrawn. `withdrawn=1` returns ONLY those — how a provider learns to drop content the owner deleted while it was not listening, bounded by the retention window rather than by how much it holds |
