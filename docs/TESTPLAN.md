...
| T5 | Recovery after wipe (1 relay down) | ☑ | **Green on v3, 2026-08-15.** Fresh-device recovery via the face-gated release (two relay-drawn sequences, shares combined) — once with all three custodians up (used local x=1 + cloud x=2, never contacting the third: any-2-of-3 proven), and once with relay-2 stopped (`distance=0.158 margin=0.292 PASS`). Photo attempt fails at the action steps before the relay gate is even reached. ⚠ Creating an account while a relay is down yields **2-of-2 — no redundancy** (see the share-refresh gap below); lockout-survives-a-wipe still unrun |
