...
| T8 | Storage provider lifecycle | ☑ | **ALL STEPS PASS** (2026-09-21, `e2e/provider-lifecycle.spec.ts`): register shows declared capacity and a **zero** rate (capacity earns nothing — you are paid for bytes held), the other device discovers it via `GET /providers` and renders `—` for everything it cannot know, an immediate second heartbeat is refused with `interval not reached (next in 2min)`, re-registering after a deregister is **still** refused (the clock does not reset), and the reward issues non-zero one epoch behind with a duplicate claim refused. Two things the run taught: steps 1/3/4 must be ONE ordered test (the interval is 2 min under `fast`, so split up the "early" heartbeat is no longer early), and the reward needs bytes worth paying for — `1000 × storedGB × counted/6` truncates to zero below ~1 MB held |
| T9 | Publish handoff + repair | ☑ | **ALL STEPS PASS** (2026-09-21, `e2e/custody.spec.ts`). Step 1 needs **three** accounts (MIN_REPLICAS is 2 and the uploader never counts itself). Steps 3+4 were unreachable for three separate reasons, all now fixed — the publisher never dropped its copy, the availability check inherited a 10-minute deadline, and the repair trigger was keyed on "nothing came back" rather than "content unavailable". Step 6 is read off the RENDERED holder count (`0 live (1 ever)`) instead of a log line behind an early return and a backoff. Run steps 5+6 isolated (`-g "steps 5"`) if a full-file run stalls on account creation |
| T10 | File index is no longer global | ☑ | **ALL STEPS PASS** (2026-09-21, `e2e/file-index.spec.ts`): archives answer with their own `total`, a client holds only its own files, the one-time migration drops a planted foreign record once and not again, the chip reads `—` rather than a bare 0, and a withdrawal lands as a `removed:true` tombstone. The earlier "archive never received it" failures were a **truncated CID in the harness**, not delivery — step 2b now passes in 1.2 s |
