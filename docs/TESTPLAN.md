...
| T8 | Storage provider lifecycle | ☑ | **Steps 1–4 PASS** (2026-09-21, `e2e/provider-lifecycle.spec.ts`): register shows declared capacity and a **zero** rate (capacity earns nothing — you are paid for bytes held), the other device discovers it by `GET /providers` and renders `—` for everything it cannot know, an immediate second heartbeat is refused with `interval not reached (next in 2min)`, and re-registering after a deregister is **still** refused — the clock does not reset. Steps 1/3/4 must run as one ordered sequence: the interval is 2 min under `fast`, so split up, the "early" heartbeat is no longer early. Step 5 (reward) runs under `fast` and skips on the production clock |
